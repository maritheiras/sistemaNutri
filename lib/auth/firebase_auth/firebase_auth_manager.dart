...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Algo deu errado. Verifique se preencheu todos os campos corretamente.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'Link para reset de senha enviado para o e-mail informado.')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : SistemaNutriFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'E-mail já cadastrado.',
        'INVALID_LOGIN_CREDENTIALS' =>
          'As credênciais estão incorretas. Por favor, verifique!',
        _ =>
          'Algo deu errado. Verifique se preencheu todos os campos corretamente.',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
